...
所以同步时序逻辑电路就是在一个具有回路的逻辑电路的基础上增加了D触发器保证可以在CLK上升沿或者下降沿时切换状态的一个时序电路，他就是由组合逻辑电路+触发器组成的（也可以是寄存器）。其实同步时序逻辑电路的本质特定就是他是在一个特定的CLK周期内周期性的切换电路状态。
